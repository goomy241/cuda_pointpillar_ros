...
PointPillarWrapper::PointPillarWrapper(): elapsedTime(0.0f) {
    Getinfo();
...
PointPillarWrapper::~PointPillarWrapper() {
    checkCudaErrors(cudaEventDestroy(start));
    checkCudaErrors(cudaEventDestroy(stop));
    checkCudaErrors(cudaStreamDestroy(stream));
}

void PointPillarWrapper::run() {
    Params params_;
...
void PointPillarWrapper::Getinfo(void)
{
cudaDeviceProp prop;
...
int PointPillarWrapper::loadData(const char *file, void **data, unsigned int *length)
{
...
void PointPillarWrapper::SaveBoxPred(std::vector<Bndbox> boxes, std::string file_name)
{
    std::ofstream ofs;
